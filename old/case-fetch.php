...
//Initial Value
$database_iso_select='gl';
$date_accuracy_select='td';
...
//Last Update Assign
//$today=new DateTime($last_date);
$cprev_day=$prev_day->modify('+1 day');
$bulan=$cprev_day->format('m');
if($bulan<'10'){
    $bulan=$bulan[1];
}
...
foreach($isocode_stcode as $rg_code){
    ...
    $increment_case=$last_data-$prev_data;
    $inc_fpd=round($increment_case-$pd_increment_case,2);
    if($inc_fpd>=0){
        $inc_fpd='+'.$inc_fpd;
    }
...
foreach($isocode_stcode as $rg_code){
    ...
    $increment_case=round(($last_data-$prev_data)/$prev_data,4)*100;
    $prev_increment=round($increment_case-$last_increment_case,2);
    if($prev_increment>=0){
